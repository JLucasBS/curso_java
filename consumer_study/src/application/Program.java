package application;

import java.util.ArrayList;
import java.util.List;
import java.util.Locale;

import entities.Product;

public class Program {

	public static void main(String[] args) {
		
		Locale.setDefault(Locale.US);
		List<Product> list = new ArrayList<>();
		
		list.add(new Product("Tv", 900.00));
		list.add(new Product("Mouse", 50.00));
		list.add(new Product("Tablet", 350.50));
		list.add(new Product("HD Case", 80.90));
		
//		list.forEach(new PriceUpdate()); -> implementação da interface
//		list.forEach(Product::staticPriceUpdate); -> reference method com metodo estatico
//		list.forEach(Product::nonStaticPriceUpdate); -> reference method com metodo não estatico
		
		double factor = 1.1;
		
//		Consumer<Product> cons = p -> p.setPrice(p.getPrice() * factor); -> expressão lambda declarada
//		list.forEach(cons);
		
		list.forEach(p -> p.setPrice(p.getPrice() * factor)); // -> expressão lambda inline
		
		
		list.forEach(System.out::println);
	}

}
